...
namespace Power_Mplayer
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
        ...
        private MenuItem MI_OpenVCDISO;
        private MenuItem MI_OpenDVDISO;
        private MplayerWrapper.MplayerControl BigScreen;
        private Label txtStatus;

        private MplayerControl mp;
		...
		private Form1()
		{
            ...
            BigScreen.Setting = ms;
            BigScreen.OnReceiveMediaInfo += SetTimePos;
            mp = BigScreen;

            string var = BigScreen.Setting[SetVars.SubEncoding];
            ...
            var = BigScreen.Setting[SetVars.SubChineseTrans];
		...
		private void InitializeComponent()
		{
            ...
            this.BigScreen = new MplayerWrapper.MplayerControl();
            ...
            this.MI_OpenVCDISO = new System.Windows.Forms.MenuItem();
            this.MI_OpenDVDISO = new System.Windows.Forms.MenuItem();
            ...
            this.BigScreen.Percent_Pos = 0;
            this.BigScreen.Setting = null;
            this.BigScreen.Time_Pos = 0;
            this.BigScreen.Volume = 0;
            this.BigScreen.DoubleClick += new System.EventHandler(this.BigScreen_DoubleClick);
            this.BigScreen.MouseMove += new System.Windows.Forms.MouseEventHandler(this.MainPanel_MouseMove);
            ...
            // MI_OpenDVD
            // 
            this.MI_OpenDVD.Index = 2;
            resources.ApplyResources(this.MI_OpenDVD, "MI_OpenDVD");
            this.MI_OpenDVD.Click += new System.EventHandler(this.MI_OpenDVD_Click);
            // 
            // MI_OpenVCDISO
            // 
            this.MI_OpenVCDISO.Index = 3;
            resources.ApplyResources(this.MI_OpenVCDISO, "MI_OpenVCDISO");
            this.MI_OpenVCDISO.Click += new System.EventHandler(this.MI_OpenVCDISO_Click);
            // 
            // MI_OpenDVDISO
            // 
            this.MI_OpenDVDISO.Index = 4;
            resources.ApplyResources(this.MI_OpenDVDISO, "MI_OpenDVDISO");
            this.MI_OpenDVDISO.Click += new System.EventHandler(this.MI_OpenDVDISO_Click);
            // 
            // MI_OpenURL
            // 
            this.MI_OpenURL.Index = 5;
            ...
            // 
            this.MI_AudioBalance_Center.Checked = true;
        ...
        private Bitmap DrawPanelBackground(int height)
        {
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                Color c1 = Color.FromArgb(130, c2);

                Rectangle rect = new Rectangle(0, 0, 1, height);

                rect.Y = MovieBar.Height;
                rect.Height = (rect.Height - rect.Y) / 2 + 10;

                Brush linearBrush = new LinearGradientBrush(rect, c1, c2, LinearGradientMode.Vertical);
                rect.Height *= 2;
                Brush linearBrush2 = new LinearGradientBrush(rect, c2, c1, LinearGradientMode.Vertical);
                rect.Height = (rect.Height - rect.Y) / 2 + 10;

                height -= MovieBar.Height;

                rect.Y = MovieBar.Height;
                rect.Height = height / 2 + 1;

                g.FillRectangle(linearBrush, 0, MovieBar.Height, 1, rect.Height);
                g.FillRectangle(linearBrush2, 0, MovieBar.Height + rect.Height, 1, height);
		...
		private void Form1_Load(object sender, System.EventArgs e)
		{
            ...
            panel1.BackgroundImage = this.DrawPanelBackground(panel1.Height);
        ...
        public void adjBigScreen()
        {
            ...
            {
                newWid = MainPanel.Width;
                newHei = (int)(newWid / aspect);
            }
            else
            {
                newHei = MainPanel.Height;
                newWid = (int)(newHei * aspect);
            }

            // update Size : Width, Height

            if (BigScreen.Width != newWid || BigScreen.Height != newHei)
                BigScreen.Size = new Size(newWid, newHei);

            // update location : Top, Left

            newWid = (MainPanel.Width - newWid) / 2;
            newHei = (MainPanel.Height - newHei) / 2;

            if (BigScreen.Left != newWid || BigScreen.Top != newHei)
                BigScreen.Location = new Point(newWid, newHei);
        }
        ...
        private void Start(MPlaylistItem item)
		{
			...
			if(mp.Start(item, start_pos))
			{
                ...
                // set balance
                MenuItem mi = (MI_AudioBalance_Center.Checked) ? MI_AudioBalance_Center : 
                    (MI_AudioBalance_Left.Checked ? MI_AudioBalance_Left : MI_AudioBalance_Right);
                this.MI_AudioBalance_Click(mi, null);
        ...
        {
            if (str.StartsWith("Generating Index"))
            {
                // safe Thread
        ...
        private void AppendAudioIDMenuItem(MenuItem owner)
        {
            ...
            if (mp.minfo.AudioChannel.Count > 0)
            {
                foreach (int j in mp.minfo.AudioChannel)
                {
        ...
        private void MI_AudioBalance_Click(object sender, EventArgs e)
        {
            ...
            MI_AudioBalance_Center.Checked = MI_AudioBalance_Left.Checked = MI_AudioBalance_Right.Checked = false;
            mi.Checked = true;
        ...
        private void MI_OpenVCD_Click(object sender, EventArgs e)
        {
            ...
            string[] files = MplayerControl.getVCDTracks(mp.Setting[SetVars.MplayerExe], volumeName);
        ...
        private void MI_OpenDVD_Click(object sender, EventArgs e)
        {
            DialogResult result = this.folderBrowserDialog1.ShowDialog();
            ...
            DriveInfo di = new DriveInfo(volumeName);
            if (di.DriveFormat != "UDF")
            {
                MessageBox.Show("Please put DVD disk.");
                return;
            }

            string[] files = MplayerControl.getDVDTitles(mp.Setting[SetVars.MplayerExe], volumeName);

            this.Playlist.Items.Clear();
            this.Playlist_AddItem(MediaType.DVD, files);

            Start(Playlist_First());
        }

        private void MI_OpenVCDISO_Click(object sender, EventArgs e)
        {
            ...
            string path = Win32API.ToShortPathName(openFileDialog1.FileName);

            string[] files = MplayerControl.getVCDTracks(mp.Setting[SetVars.MplayerExe], path);
        ...
        private void MI_OpenDVDISO_Click(object sender, EventArgs e)
        {
            DialogResult result = this.openFileDialog1.ShowDialog();
            ...
            string[] files = MplayerControl.getDVDTitles(mp.Setting[SetVars.MplayerExe], path);
